...
onMounted(async () => {
  try {
    config.value = await apiRequest("GET", `/api/databases/${dbName.value}/config`);
  ...
  <div>
    ...
    <div v-else class="space-y-8 max-w-lg">
      <div>
        ...
        <div class="flex flex-wrap gap-2 mb-2">
          <span v-for="(origin, i) in config.cors_origins" :key="i" class="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-gray-800 text-xs text-gray-300">
            {{ origin }}
            <button @click="config.cors_origins.splice(i, 1)" class="text-gray-600 hover:text-red-400">&times;</button>
          </span>
        </div>
        <div class="flex gap-2">
          <input v-model="config.cors_origins" list="cors-list" class="input flex-1" placeholder="https://example.com" @keydown.enter.prevent="() => { if(typeof config.cors_origins === 'string') { const v = config.cors_origins; config.cors_origins = [v]; } else { config.cors_origins.push(''); config.cors_origins = [...config.cors_origins]; } }" />
        </div>
      </div>
      ...
      <div>
        ...
        <input v-model.number="config.max_body_size" type="number" class="input" placeholder="1048576" />
      </div>
